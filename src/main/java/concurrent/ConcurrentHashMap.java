...
//		Node<K, V>[] tab;
//		int sc;
//		while ((tab = table) == null || tab.length == 0) {//table为空或者长度为0
//			if ((sc = sizeCtl) < 0)//别的线程已经在初始化
//				Thread.yield();//让出cpu时间等他们初始话完成
//			else if (U.compareAndSwapInt(this, SIZECTL, sc, -1)) {//获取锁？
//				try {
//					if ((tab = table) == null || tab.length == 0) {//如果获取了锁？，还是table为空或者长度为0
//						int n = (sc > 0) ? sc : DEFAULT_CAPACITY;
//						@SuppressWarnings("unchecked")
//						Node<K, V>[] nt = (Node<K, V>[]) new Node<?, ?>[n];//就new一个table
//						table = tab = nt;
//						sc = n - (n >>> 2);//计算table的容量
//					}
//				} finally {
//					sizeCtl = sc;//把容量赋值给sizeCtl，因为sizeCtl在table非初始话或者拓展时表示的含义就是容量？
//				}
//				break;
//			}
//		}
